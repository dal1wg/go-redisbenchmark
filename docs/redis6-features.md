...
```bash
# 使用Redis URI进行基准测试
./go-redisbenchmark --uri "redis://benchmark:mypassword@127.0.0.1:6379/0" -n 10000 -c 10

# 使用Redis URI连接集群
./go-redisbenchmark --uri "redis://app1:apppass@127.0.0.1:7000/0" -mode cluster -n 10000
...
```bash
# 使用TLS连接到Redis服务器
./go-redisbenchmark --tls --tls-ca certs/ca.crt -n 10000 -c 10
...
```bash
# 启用详细日志
./go-redisbenchmark --tls --tls-ca certs/ca.crt -n 1000 -c 1 -q
```
